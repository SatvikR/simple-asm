...
fib:						; calculates the rdi'th fibonacci number
	push	rbp
	mov	rbp, rsp

	sub	rsp, 8				; make room for variables
	mov	QWORD [rbp-8], rdi		; move num argument onto stack

	cmp	QWORD [rbp-8], 1		; compare num and 1
	jbe	.fib.L1				; handle case if num <= 1

	sub	rsp, 8				; make room for 1 more number
	mov	rdi, QWORD [rbp-8]		; move num into rdi
	sub	rdi, 1				; decrement the number
	call	fib				; recurse
	mov	QWORD [rbp-16], rax		; save result

	mov	rdi, QWORD [rbp-8]		; move num into rdi
	sub	rdi, 2				; decrease the number by 2
	call	fib				; recurse

	add	rax, QWORD [rbp-16]		; put some of two recursive calls in return register
	jmp	.fib.L2				; exit function
.fib.L1:
	mov	rax, QWORD [rbp-8]		; move num into return register
...
.main.L1:
	...
	call	fib			; calcualte fibonacci number
	mov	rdi, rax		; move result into rdi
	call	println			; print out result
